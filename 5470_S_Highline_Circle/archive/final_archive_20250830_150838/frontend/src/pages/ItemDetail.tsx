...
export default function ItemDetail() {
  ...
  const updateMutation = useMutation({
    mutationFn: (updates: any) => {
      console.log('ItemDetail updateMutation called with:', updates);
      return api.updateItem(id!, updates);
    },
    onSuccess: (data) => {
      console.log('ItemDetail update successful:', data);
      queryClient.invalidateQueries({ queryKey: ['item', id] });
      queryClient.invalidateQueries({ queryKey: ['items'] });
      setIsEditing(false);
    },
    onError: (error) => {
      console.error('ItemDetail update failed:', error);
    },
  });
  ...
  const handleDecisionChange = (decision: string) => {
    console.log('Changing decision to:', decision);
