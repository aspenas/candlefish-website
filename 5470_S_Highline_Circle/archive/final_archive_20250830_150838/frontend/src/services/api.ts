  ...
  async (error: AxiosError) => {
    ...
    if ((error.response?.status === 404 || error.response?.status === 500) || !error.response) {
      ...
      if (url.includes('/activities')) {
        console.warn('Activities table not available, returning mock data');
            ...
            activities: [
              {
                id: "1",
                action: "created",
                item_name: "System Initialized",
                details: "Activities tracking will be available once the database migration is complete",
...
export const api = {
  ...
  getItem: (id: string) => {
    ...
    return apiClient.get(`/items/${id}`).then(response => {
      console.log('getItem response:', response.data);
      // The backend returns {item: null} when not found, but we need to handle the actual item data
      if (response.data && response.data.item) {
        return { ...response, data: response.data.item };
      }
      // Try to find the item from the items list endpoint as fallback
      return apiClient.get('/items').then(allItemsResponse => {
        const items = allItemsResponse.data.items || allItemsResponse.data || [];
        const foundItem = items.find((item: any) => item.id === id);
        if (foundItem) {
          console.log('Found item in items list:', foundItem);
  ...
  updateItem: (id: string, data: any) => {
    ...
    return apiClient.put(`/items/${id}`, snakeCaseData);
  },
  ...
  searchItems: async (params: any) => {
    try {
      ...
      // Get all items and perform client-side filtering
      const allItems = await apiClient.get('/items');
      const items = allItems?.data?.items || allItems?.data || [];
  ...
  filterItems: (params: URLSearchParams) => {
    // The /filter endpoint has database issues, use /items instead
    console.warn('Using /items endpoint instead of broken /filter endpoint');
    const paramsObj = Object.fromEntries(params);
    return apiClient.get('/items', { params: paramsObj });
  },
