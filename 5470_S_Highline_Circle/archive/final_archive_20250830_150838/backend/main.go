...
func main() {
	...
	if err != nil {
		...
		log.Println("Auth features will be disabled")
		jwtManager = nil
	}
	...
	// Initialize auth handler only if database and JWT manager are available
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"service": "5470 S Highline Circle Inventory API",
			"version": "2.0",
			"status": "operational",
			"endpoints": fiber.Map{
				"api": "/api/v1",
			...
			"documentation": "https://5470-inventory.fly.dev/swagger",
			"frontend": "https://inventory.highline.work",
			"nanda": "http://localhost:5100",
		})
	})
